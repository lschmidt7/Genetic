...
class Population():
    ...
    def fitness(self):
        ...
        i=0
        for x in self.population:
            print(i/len(self.population)*100.0)
    ...
    def crossover(self):
        ...
        for x in range(int(len(comunity)/2)):
            ...
            point = random.randint(1,(self.size_chromosome-1))*9 # CUIDADO
    ...
    def mutation(self):
        for x in range(len(self.population)):
            ...
            if(sorting<=self.mutation_rate):
                #index = random.randint(0,self.size_chromosome-1)*9
                self.population[x].randomize(self.values)